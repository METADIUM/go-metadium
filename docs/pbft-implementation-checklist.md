...
| M-10 | Sidecar fetch before PREPARE: a full-blob block (6 × 128 KiB) fetched within `sidecarWait` (2 s) on the event loop, with validators across regions; if it falls short, scale the wait with the blob count or make it a setting, and remember peers that answer empty for the height (review on #163) | recorded | Not exercised on a LAN. `metabft/proposal/sidecars` has no samples over the runs: the pool delivers blob sidecars to every validator before the proposal (`blob-tx-e2e` passes on PBFT). The timer is in place for a cross-region run | [ ] — needs validators across regions |
