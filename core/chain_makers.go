...
func (b *BlockGen) SetParentBeaconRoot(root common.Hash) {
	// ParentBeaconRoot is not supported in Metadium header format; skip assignment
	var (
...
func (b *BlockGen) addTx(bc *BlockChain, vmConfig vm.Config, tx *types.Transaction) {
	...
	receipt, err := ApplyTransaction(b.cm.config, bc, &b.header.Coinbase, b.gasPool, b.statedb, b.header, tx, &b.header.GasUsed, nil, vmConfig)
	if err != nil {
		panic(err)
	}
	...
	if b.header.BlobGasUsed != nil {
		b.header.BlobGasUsed.Add(b.header.BlobGasUsed, new(big.Int).SetUint64(tx.BlobGas()))
	}
}
...
func (cm *chainMaker) makeHeader(parent *types.Block, state *state.StateDB, engine consensus.Engine) *types.Header {
	...
	if cm.config.IsCancun(header.Number, header.Time) || (cm.config.IsCamellia(header.Number) && isPoS) {
		...
		if parentHeader.ExcessBlobGas != nil {
			parentExcessBlobGas = parentHeader.ExcessBlobGas.Uint64()
		}
