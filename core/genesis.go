...
import (
	...
	triedbpkg "github.com/ethereum/go-ethereum/triedb"
...
func ReadGenesis(db ethdb.Database) (*Genesis, error) {
	...
	if genesisHeader.ExcessBlobGas != nil {
		v := genesisHeader.ExcessBlobGas.Uint64()
		genesis.ExcessBlobGas = &v
	}
	if genesisHeader.BlobGasUsed != nil {
		v := genesisHeader.BlobGasUsed.Uint64()
...
func hashAlloc(ga *types.GenesisAlloc, isVerkle bool) (common.Hash, error) {
	...
	// as such.
	var config *triedbpkg.Config
	if isVerkle {
		config = &triedbpkg.Config{
...
func flushAlloc(ga *types.GenesisAlloc, db ethdb.Database, triedb *triedbpkg.Database, blockhash common.Hash) error {
	// Create a default hash-scheme triedb when none is provided (e.g. in tests).
	if triedb == nil {
		triedb = triedbpkg.NewDatabase(db, nil)
	}
...
func SetupGenesisBlock(db ethdb.Database, triedb *triedbpkg.Database, genesis *Genesis) (*params.ChainConfig, common.Hash, error) {
	return SetupGenesisBlockWithOverride(db, triedb, genesis, nil)
}

func SetupGenesisBlockWithOverride(db ethdb.Database, triedb *triedbpkg.Database, genesis *Genesis, overrides *ChainOverrides) (*params.ChainConfig, common.Hash, error) {
...
func (g *Genesis) ToBlock() *types.Block {
	...
	if conf := g.Config; conf != nil {
		...
		if conf.IsCancun(num, g.Timestamp) {
			// EIP-4844 fields (Metadium uses *big.Int for these fields)
			if g.ExcessBlobGas != nil {
				head.ExcessBlobGas = new(big.Int).SetUint64(*g.ExcessBlobGas)
			} else {
				head.ExcessBlobGas = new(big.Int)
			}
			if g.BlobGasUsed != nil {
				head.BlobGasUsed = new(big.Int).SetUint64(*g.BlobGasUsed)
			} else {
				head.BlobGasUsed = new(big.Int)
			}
			// Note: ParentBeaconRoot not in Metadium header format
		}
	}
...
func (g *Genesis) Commit(db ethdb.Database, triedb *triedbpkg.Database) (*types.Block, error) {
	block := g.ToBlock()
...
// The block is committed as the canonical head block.
func (g *Genesis) MustCommit(db ethdb.Database, triedb *triedbpkg.Database) *types.Block {
...
// DefaultGenesisBlock returns the Metadium main net genesis block.
...
// DefaultTestnetGenesisBlock returns the Metadium testnet genesis block.
