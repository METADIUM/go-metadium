...
func (evm *EVM) create(caller ContractRef, codeAndHash *codeAndHash, gas uint64, value *uint256.Int, address common.Address, typ OpCode) ([]byte, common.Address, uint64, error) {
	...
	// EIP-3860: Reject oversized init code (after nonce increment, per EIP spec).
	// Gas metering for init code word cost is handled in the opcode gas functions
	// (gasCreateEip3860 / gasCreate2Eip3860) before create() is called.
	if evm.chainRules.IsShanghai && len(codeAndHash.code) > int(params.MaxInitCodeSize) {
