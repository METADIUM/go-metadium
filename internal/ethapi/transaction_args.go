...
func (args *TransactionArgs) setCancunFeeDefaults(ctx context.Context, head *types.Header, b Backend) error {
	...
	if args.BlobHashes != nil && args.BlobFeeCap == nil {
		...
		if head.ExcessBlobGas != nil {
			excessBlobGas = head.ExcessBlobGas.Uint64()
		}
...
func (args *TransactionArgs) toTransaction() *types.Transaction {
	...
	case args.BlobHashes != nil:
		...
		data = &types.BlobTx{
			...
			AccessList:       al,
			BlobHashes:       args.BlobHashes,
			MaxFeePerBlobGas: uint256.MustFromBig((*big.Int)(args.BlobFeeCap)),
		}
		if args.Blobs != nil {
			blobs := make([][]byte, len(args.Blobs))
			for i, b := range args.Blobs {
				blobs[i] = b[:]
			}
			commitments := make([][]byte, len(args.Commitments))
			for i, c := range args.Commitments {
				commitments[i] = c[:]
			}
			proofs := make([][]byte, len(args.Proofs))
